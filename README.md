...
This repository implements a minimal Vector DB in C++ from scratch featuring **Scalar Quantization (SQ8)** and **hardware SIMD acceleration**, bridges it to Python, and wraps it in a **Go + Python microservices-based interactive Web interface** utilizing a local **Gemma-2-2b-it** LLM for RAG.
...
├── lib/                        # Destination for compiled native shared libraries (C++ modules)
├── models/                     # Directory storing local LLM models (e.g. Gemma GGUF)
...
├── download_model_gguf.py      # Downloader script for Gemma-2-2b GGUF
...
5. **Go & Python Microservices Architecture**: 
   ...
   - A **Python FastAPI** service runs in the background handling embedding generation via HuggingFace models, querying the native C++ DB, and communicating with a local **Gemma-2-2b-it GGUF** model using **llama-cpp-python**.
...
Make sure you compile from the root directory and place the output binary inside the `lib/` folder:
```bash
# 1. Setup and activate Virtual Environment
python3 -m venv venv
source venv/bin/activate

# 2. Install dependencies (with Metal support on macOS)
CMAKE_ARGS="-DLLAMA_METAL=on" pip install pybind11 sentence-transformers fastapi uvicorn llama-cpp-python

# 3. Compile the C++ shared library for Python into lib/
mkdir -p lib
c++ -O3 -Wall -shared -std=c++17 -fPIC -undefined dynamic_lookup $(python -m pybind11 --includes) core/nano_vdb.cpp core/binding.cpp -o lib/nano_vdb$(python -c "import sysconfig; print(sysconfig.get_config_var('EXT_SUFFIX'))")
```

### 2. Download the LLM Model
Download **Gemma-2-2b-it** GGUF into the `models/` directory:
```bash
